...
const uri = `mongodb+srv://carService:carService1218@cluster0.p4mie.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    app.post("/orders", verifyJWT, async (req, res) => {
      ...
      const { service, email, address, customer } = order;
      if (!service || !email || !address || !customer) {
    ...
    app.get("/orders/by-transaction-id/:id", async (req, res) => {
      ...
      if (order) {
        // If order exists, send it as a JSON response
        res.json(order);
      } else {
        // If order doesn't exist, send an error response with a meaningful message
